class VisitorsController < ApplicationController
  ...
  # GET /visitors/new
  def new
    @visitor = current_user.group.churches.first.visitors.build
  end
  ...
  # POST /visitors.json
  def create
    @visitor = current_user.group.churches.first.visitors.build(visitor_params)

    respond_to do |format|
      if @visitor.save
        format.html { redirect_to church_path(@visitor.church_id), notice: 'Successfully signed in.' }
  ...
  private
    ...
    def visitor_params
      params.require(:visitor).permit(:name, :age, :church_id)
    end
end
